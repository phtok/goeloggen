...
export async function bauePdf(S, f, bilder, orte, lage, grad, logoSvg) {
  const doc = new jspdf.jsPDF({ unit: 'mm', format: [SB, SH], orientation: 'portrait', compress: true });
  ...
  doc.addImage(bilder.titel.url, 'JPEG', 0, bildY, SB, bildH, 'titel', 'SLOW');
  const k = 14, fh = 126;
  const kante = { schraege: [[0, 0], [SB, 0], [SB, fh - k], [0, fh]],
    trapez: [[0, 0], [SB, 0], [SB, fh - k], [SB * 0.78, fh], [SB * 0.22, fh], [0, fh - k]],
    gerade: [[0, 0], [SB, 0], [SB, fh - k], [0, fh - k]] }[S.kante] || [[0, 0], [SB, 0], [SB, fh], [0, fh]];
  if (S.kante === 'gerade') { doc.setFillColor(WEISS); doc.rect(0, bildY, SB, fh - k - bildY, 'F'); }
  ...
  doc.addImage(bilder.plan.url, 'PNG', kx, kastenY, kb, kh, 'plan', 'SLOW');
