...
## Zwei Werkstätten, eine Quelle (seit 16. 9. 2026)

Dieses xDev-Projekt («Grafik-Werkstatt») ist eine Arbeitskopie des Repos
https://github.com/phtok/goeloggen – mit vollem Verlauf, Remote `github`.

- **Veröffentlicht wird weiterhin über GitHub:** `main` dort treibt GitHub
  Pages (https://werkzeuge.goetheanum.ch) und die Supabase-Edge-Functions
  (`.github/workflows/`). Arbeit aus dieser Werkstatt ist erst live, wenn sie
  nach `github/main` gepusht ist. Dafür liegt ein Fine-grained-PAT (nur dieses
  Repo, Contents R+W) als `GITHUB_TOKEN` in `.env` – nie im Code, nie im Chat.
- **Die Vorschau hier** zeigt die Wurzel-`index.html` des rohen Repo-Stands,
  wie der Branch-Lauf von Pages. Erzeugte Pfade (Abschnitte aus
  `sektionen.json`, Kurz-Adressen je Werkzeug) sind eingecheckt und gelten
  darum auch hier. Neu erzeugen nach Änderung von `sektionen.json`/`tools.json`:
  `python3 tools/build_sections.py .` und `python3 tools/build_tool_aliases.py .`
- **Ablage nach Plattform-Konvention:** `CLAUDE.md` ist ein Link auf diese
  Datei; Skills wohnen in `project-skills/` (`.claude/skills` und
  `.agents/skills` zeigen dorthin). Die Agenten bleiben in `.claude/agents/`.
- Der Abschnitt «Pull Requests» unten gilt für Arbeit direkt im GitHub-Repo.
  Hier faltet die Plattform jede Sitzung selbst nach `main`; der Prüf-Hook
  (`git config core.hooksPath tools/hooks`) läuft auch hier bei jedem Commit.
